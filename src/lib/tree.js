...
function Tree(sizeBranch, sizeTrunk, radius, scene, sd) {
    "use strict";
    ...
    var branchColor = randomColor({
        hue: 'green',
        luminosity: 'bright',
        format: 'rgbArray'
    });
    var trunkColor = randomColor({
        hue: 'orange',
        luminosity: 'dark',
        format: 'rgbArray'
    });
...
Tree.prototype._init = function (sizeBranch) {
    ...
    for (i = 0; i < numberOfPoints; i++) {
        ...
        for (index = 0; index < map.length && !found; index++) {
            array = map[index];
            var p0 = array[0];
